...
void* my_malloc(size_t size) {
    ...
    struct Node* new_node = (struct Node*)((char*)best + sizeof(struct Node) + size);
...
void dump() {
    ...
    while (tmp != NULL) {
        ...
        printf("Ptr: %p, size: %zu, count_sz: %zu", tmp, tmp->size_, 
...
void my_free_1(void* ptr) {
    ...
    if (list.head_ == ptr) {
        ...
        if (list.head_->next_->size_ != 0) {
            ...
            list.head_->size_ = 
            (char*)(list.head_->next_) - (char*)(list.head_) - sizeof(struct Node);
        }
    ...
    if (prev->next_ == NULL) {
        ...
        prev->size_ = (char*)list.head_ + list.size_ - (char*)prev - sizeof(struct Node);
